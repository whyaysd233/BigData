...
import re
# SPARK下实现从分词后的文本中提取出腐败行为环节的特征
# 输入：指定目录下的分词后文本 格式：<原判决书文本路径，文本分词结果（空格分离）>的键值对
#       特征词文件 keywords_process.txt 格式：<维度,该维度对应的可能特征词>
# 输出：文件格式：<判决书ID，最显著的行为环节维度，对应特征词词频>
...
def main():
    ...
    # 从keywords_process.txt中提取出各维度对应的特征词，存成<维度，set(特征词)>键值对
    semicolon = "；"
    keywords_raw = sc.textFile("/home/djt/data/keywords_process.txt")
